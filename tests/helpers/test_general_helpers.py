...
def test_get_fit_to_pick_samples_count_valid_barcode(app, samples, priority_samples):
    with app.app_context():
        ...
        if fit_to_pick_samples:
            ...
            assert count_must_sequence == 4
            assert count_preferentially_sequence == 4
            assert count_filtered_positive == 4
        else:
...
def test_get_fit_to_pick_samples_count_invalid_barcode(app, samples):
    with app.app_context():
        assert get_fit_to_pick_samples_and_counts("abc") == (None, None, None, None, None)
