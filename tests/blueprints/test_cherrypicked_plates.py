...
def test_post_cherrypicked_plates_endpoint_successful(app, client, samples, mocked_responses, mlwh_lh_samples):
    with patch(
        ...
        response = client.post(
            "/cherrypicked-plates/create", data=json.dumps({"barcode": "123"}), content_type="application/json",
        )
...
def test_post_cherrypicked_plates_endpoint_no_barcode_in_request(app, client, samples):
    response = client.post("/cherrypicked-plates/create", data=json.dumps({}), content_type="application/json",)
...
def test_post_plates_mlwh_update_failure(app, client, samples, mocked_responses):
    with patch(
        ...
        with patch(
            "lighthouse.blueprints.cherrypicked_plates.update_mlwh_with_cog_uk_ids", side_effect=Exception('Boom!')
        ):
            ss_url = f"http://{app.config['SS_HOST']}/api/v2/heron/plates"
