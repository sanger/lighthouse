...
def test_post_error_recovered_destination_completed_missing_barcode(app, client, biosero_auth_headers, clear_events):
    with app.app_context():
        response = client.post(
            "/events",
            data={"automation_system_run_id": 123, "event_type": Biosero.EVENT_ERROR_RECOVERED_DESTINATION_COMPLETED},
...
def test_post_error_recovered_destination_completed_cherrytrack_fails(
    app,
    client,
    ...
    with app.app_context():
        response = client.post(
            ...
            data={
                ...
                "barcode": "HT-1234",
                "event_type": Biosero.EVENT_ERROR_RECOVERED_DESTINATION_COMPLETED,
            },
            headers=biosero_auth_headers,
        )
...
@pytest.mark.parametrize("source_barcode", ["plate_123"])
@pytest.mark.parametrize("destination_barcode", ["HT-1234"])
def test_post_event_recovered_destination_completed(
    app,
    client,
    biosero_auth_headers,
    clear_events,
                ...
                with patch(
                    ...
                    with patch(
                        ...
                        mocked_rabbit_channel.basic_publish.assert_called_with(
                            ...
                            routing_key=f"test.event.{ Biosero.EVENT_ERROR_RECOVERED_DESTINATION_COMPLETED }",
                            body=event_message,
                        )
...
@pytest.mark.parametrize("destination_barcode", ["HT-1234"])
def test_post_event_recovered_destination_completed_without_cog_uk_ids(
    app,
    client,
    biosero_auth_headers,
    clear_events,
